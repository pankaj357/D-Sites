#!/usr/bin/env python3
"""
Validate TFBS Predictions Against Known Regulated Genes (FIMO-style)
...
4. Check if predicted TFBS (FIMO CSV) fall within promoters.
...
def make_visualizations(results_df, total_genes, mapped_count, promoter_hit_count, out_prefix):
    ...
    # Histogram of distances from TSS
    plt.figure(figsize=(6, 4))
...
def main(gff_file, gene_file, pred_file, out_prefix, upstream, downstream):
    ...
    pred_df = pd.read_csv(pred_file, sep="\t")  # FIMO TSV

    pred_df.rename(columns=lambda x: x.strip().lower(), inplace=True)

    # Rename FIMO-specific columns
    ...
    if not required_cols.issubset(set(pred_df.columns)):
        raise ValueError(f"FIMO predictions file must have columns: {required_cols}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Validate FIMO TFBS predictions")
    ...
    parser.add_argument("--pred", required=True, help="FIMO TFBS predictions CSV")
